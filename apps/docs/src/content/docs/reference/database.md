...
| `extractions` | the structured archive — one present/absent model | `status`, `model` (JSONB), `figure_reproduced`, indexed facets |
...
All five tables have row-level security enabled with a single policy: authenticated users
have full access. The `papers` storage bucket is **public** by design — a deliberate choice
for reproducibility (anyone can trace an extracted model to its source paper).

*Source: `supabase/migrations/0001_init.sql`, `0002_job_target.sql`,
`0003_papers_storage_policies.sql`.*
